...
- Cameras with a channel get `binary_sensor` **Motion** (`device_class: motion`) from event-push `videoMotion` / `human` / `mobileDetect` / PIR / person-in-area AI (`e_aiPerArea`, `e_multiVideoAiPerArea`, `e_smartMixDetect`). It stays on for 15 seconds, or turns off immediately on a PIR-clear push. Pet / fire / gas / vehicle / line-crossing alarms stay on notify + `imou_life_alarm`. Needs event push with type **alarm**; the entity remains if push is off, but stays `off`. Distinct from **Picture change** / **Human detection** switches
...
- 有通道的摄像头增加 `binary_sensor` **动态侦测**（`device_class: motion`），由事件推送的画面变化 / 人形 / PIR / 区域人形 AI（`e_aiPerArea`、`e_multiVideoAiPerArea`、`e_smartMixDetect`）置位，约 15 秒后关，PIR 清除立即关。宠物 / 火警 / 燃气 / 车辆 / 越线仍走通知和 `imou_life_alarm`。需开启事件推送且类型含 **alarm**；关掉推送时实体还在，只是一直 `off`。与 **画面变化** / **人形检测** 开关不是同一实体
