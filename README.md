...
- **Everything unavailable / Open Platform calls used up** — **Settings → System → Repairs** explains when this App ID has no monthly calls left. Check **My Resources** in the [international](https://open.imoulife.com/consoleNew/resourceManage/myResource) or [China](https://open.imou.com/consoleNew/resourceManage/myResource) console, then reload the integration.
...
- **全部不可用 / 开放平台次数用完** — **设置 → 系统 → 修复** 会说明这个 App ID 本月调用次数已用完。到 [国内](https://open.imou.com/consoleNew/resourceManage/myResource) 或 [海外](https://open.imoulife.com/consoleNew/resourceManage/myResource) 控制台的 **我的资源** 查看，然后重新加载集成。
