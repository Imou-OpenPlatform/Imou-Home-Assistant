...
class ImouConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_login(
        ...
        try:
            ...
            _LOGGER.warning("Failed to fetch device list, creating entry without device selection")
            self._devices_map = {}
        finally:
    ...
    async def async_step_select_devices(
        ...
        # Default: all devices pre-selected
        return self.async_show_form(
...
class ImouOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if webhook_id:
            try:
                suggested_webhook_url = webhook.async_generate_url(self.hass, webhook_id)
            except Exception:  # noqa: BLE001
        ...
        return self.async_show_form(
                ...
                vol.Schema(
                    {
                        ...
                        vol.Required(PARAM_BASE_PUSH, default=DEFAULT_BASE_PUSH): vol.In(
                            {"1": "推送", "2": "不推送"}
                        ),
            ...
            description_placeholders={
                ...
                "current_webhook_url": current_webhook_url or "未填写，将使用上方建议地址/自动生成地址",
            },
            last_step=False,
        )
